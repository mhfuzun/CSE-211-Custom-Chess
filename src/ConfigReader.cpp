...
void ConfigReader::parsePieces(const nlohmann::json &json) {
  ...
  for (const auto &pieceJson : json["pieces"]) {
	...
	if (pieceJson.contains("ascii") && pieceJson.contains("display_on")) {
		const auto &ascii_arr = pieceJson["ascii"];
		if (ascii_arr.contains("white"))
			piece.display_ascii[0] = ascii_arr.value("white", "x");
		if (ascii_arr.contains("black"))
			piece.display_ascii[1] = ascii_arr.value("black", "x");

		piece.display_on = pieceJson.value("display_on", false);
	}
